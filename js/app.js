...
async function shareAnimal(animalId, cpfDigits){
  ...
  if(!targetUid){
    return { ok:false, error:'CPF não encontrado. A pessoa precisa ter uma conta criada no app.' };
  }
      ...
      <div class="modal-body">
        ${animal.sharedWithUid ? `
          <p style="font-size:13.5px;line-height:1.5;margin:0 0 16px;">
            Este prontuário está compartilhado (somente leitura) com
            <strong class="mono">${escapeHtml(animal.sharedWithCpf||'')}</strong>.
          </p>
        ...
        ` : `
          <p style="font-size:13.5px;color:var(--ink-soft);line-height:1.5;margin:0 0 14px;">
            A pessoa poderá ver este prontuário (perfil, vacinas, medicações e histórico)
            em modo somente leitura. Ela precisa já ter uma conta criada no app.
          </p>
