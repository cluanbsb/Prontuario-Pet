...
async function loadUserData(){
  try{
    ...
    // Garante que exista um jeito de outras pessoas te encontrarem pelo CPF
    // (contas criadas antes desse recurso existir também são preenchidas aqui).
    const myCpfDigits = (currentUser.displayName || '').replace(/\D/g,'');
    if(myCpfDigits){
      db.collection('cpf_lookup').doc(myCpfDigits).set({ uid: currentUser.uid }).catch(()=>{});
    }
...
async function lookupUidByCpf(cpfDigits){
  try{
    const doc = await db.collection('cpf_lookup').doc(cpfDigits).get();
...
async function syncSharedCopy(animal){
  ...
  try{
    await db.collection('compartilhados').doc(shareId).set({
      ...
      ownerCpf: currentUser.displayName || '',
...
async function shareAnimal(animalId, cpfDigits){
  const targetUid = await lookupUidByCpf(cpfDigits);
  if(!targetUid){
    return { ok:false, error:'CPF não encontrado. A pessoa precisa ter uma conta e já ter feito login pelo menos uma vez no app.' };
  }
  ...
  animal.sharedWithCpf = formatCPF(cpfDigits);
...
async function unshareAnimal(animalId){
  ...
  delete animal.sharedWithCpf;
  saveData();
}

async function openEditAccount(){
  editAccountState = { loading: true, currentEmail: '' };
  editAccountError = '';
  render();
  const cpfDigits = (currentUser.displayName || '').replace(/\D/g,'');
  let currentEmail = '';
  try{
    const doc = await db.collection('cpf_lookup').doc(cpfDigits).get();
    if(doc.exists && doc.data().email) currentEmail = doc.data().email;
  }catch(e){ /* segue sem preencher */ }
  editAccountState = { loading: false, currentEmail };
  render();
}
...
async function saveEditAccount(){
  ...
  try{
    ...
    const targetAuthEmail = newEmailRaw || cpfToPseudoEmail(cpfDigits);
    if(targetAuthEmail !== currentUser.email){
      await currentUser.updateEmail(targetAuthEmail);
    }

    if(newEmailRaw){
      await db.collection('cpf_lookup').doc(cpfDigits).set({ uid: currentUser.uid, email: newEmailRaw }, { merge:true });
    }else{
      await db.collection('cpf_lookup').doc(cpfDigits).set({
        uid: currentUser.uid,
        email: firebase.firestore.FieldValue.delete(),
      }, { merge:true });
    }

    toastMessage = newEmailRaw ? 'E-mail de recuperação atualizado.' : 'E-mail de recuperação removido.';
    ...
    editAccountError = map[e.code] || 'Ocorreu um erro. Tente novamente.';
...
function renderLoginScreen(){
  ...
  const subs = {
    ...
    forgotpw:'Enviaremos um link de redefinição para o e-mail cadastrado nesta conta.',
  };
  return `
  <div class="login-screen">
    <div class="login-card">
      ...
      ` : `
        ...
        ${authMode==='signup' ? `
          <div class="field">
            <label>E-mail de recuperação <span style="font-weight:400;color:var(--ink-soft);">(opcional)</span></label>
            <input type="email" id="auth-recovery-email" placeholder="seu@email.com" autocomplete="email">
            <p style="font-size:12px;color:var(--ink-soft);margin:6px 0 0;">Se cadastrar, você pode usar "Esqueci a senha" pra recuperar o acesso por e-mail. Sem isso, só é possível trocar a senha informando a atual.</p>
          </div>
        ` : ''}
...
function attachLoginEvents(){
  ...
  ['auth-cpf','auth-password','auth-current-password','auth-new-password','auth-new-password-confirm','auth-recovery-email'].forEach(id=>{
...
async function resolveLoginEmail(cpfDigits){
  try{
    const doc = await db.collection('cpf_lookup').doc(cpfDigits).get();
    if(doc.exists && doc.data().email) return doc.data().email;
  }catch(e){ /* sem sorte na busca — cai no e-mail padrão abaixo */ }
  return cpfToPseudoEmail(cpfDigits);
}
...
async function handleAuthSubmit(){
  ...
  try{
    if(authMode==='login'){
      const loginEmail = await resolveLoginEmail(cpfDigits);
      await auth.signInWithEmailAndPassword(loginEmail, password);
    }else{
      const recoveryEmailRaw = (document.getElementById('auth-recovery-email')?.value || '').trim();
      const hasRecoveryEmail = recoveryEmailRaw.length > 0;
      if(hasRecoveryEmail && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(recoveryEmailRaw)){
        authError = 'E-mail de recuperação inválido.'; render(); return;
      }
      const signupEmail = hasRecoveryEmail ? recoveryEmailRaw : cpfToPseudoEmail(cpfDigits);
      const cred = await auth.createUserWithEmailAndPassword(signupEmail, password);
      await cred.user.updateProfile({ displayName: formatCPF(cpfDigits) });
      const lookupData = hasRecoveryEmail
        ? { uid: cred.user.uid, email: recoveryEmailRaw }
        : { uid: cred.user.uid };
      db.collection('cpf_lookup').doc(cpfDigits).set(lookupData).catch(()=>{});
    }
  }catch(e){
...
async function handleForgotPassword(){
  const cpfDigits = document.getElementById('auth-cpf').value.replace(/\D/g,'');
  if(!isValidCPF(cpfDigits)){ authError = 'CPF inválido.'; authInfo=''; render(); return; }
  authError = '';
  try{
    const doc = await db.collection('cpf_lookup').doc(cpfDigits).get();
    const email = doc.exists ? doc.data().email : null;
    if(!email){
      authError = 'Esta conta não tem e-mail de recuperação cadastrado. Use a opção "Alterar senha" informando sua senha atual.';
      render();
      return;
    }
    await auth.sendPasswordResetEmail(email);
    authInfo = `Enviamos um link de redefinição para o e-mail cadastrado (${email.replace(/^(.{2}).*(@.*)$/, '$1***$2')}).`;
    render();
  }catch(e){
    authError = translateAuthError(e.code);
    render();
...
async function handleChangePassword(){
  ...
  if(!cpfDigits || !currentPassword || !newPassword || !newPasswordConfirm){
  ...
  try{
    const cred = await auth.signInWithEmailAndPassword(pseudoEmail, currentPassword);
...
function renderDetail(){
  ...
  <main class="detail">
    ...
    ${a.isShared ? `<div class="shared-banner">🔗 Compartilhado por <strong>${escapeHtml(a.ownerCpf||'outro usuário')}</strong> · vocês dois podem editar</div>` : ''}
    ${!a.isShared && a.sharedWithUid ? `<div class="shared-banner shared-banner-owner">🔗 Compartilhado com <strong>${escapeHtml(a.sharedWithCpf||'')}</strong> · ela(e) também pode editar</div>` : ''}
...
function renderEditAccountModal(){
  const cpfDisplay = currentUser.displayName || '';
  const wrap = document.createElement('div');
  wrap.className = 'overlay';
    ...
    <div class="modal" role="dialog" aria-modal="true">
      ...
      <div class="modal-body">
        ${editAccountState.loading ? `<p style="font-size:13.5px;color:var(--ink-soft);">Carregando...</p>` : `
          <div class="field">
            <label>CPF</label>
            <input type="text" value="${escapeAttr(cpfDisplay)}" disabled>
          </div>
          ...
          <div class="field">
            ...
            <p style="font-size:12px;color:var(--ink-soft);margin:6px 0 0;">Vincular um e-mail permite usar "Esqueci a senha" pra recuperar o acesso. Deixe em branco para remover o e-mail vinculado.</p>
          </div>
          <div class="field">
      ...
      <div class="modal-body">
        ${animal.sharedWithUid ? `
          <p style="font-size:13.5px;line-height:1.5;margin:0 0 16px;">
            ...
            <strong class="mono">${escapeHtml(animal.sharedWithCpf||'')}</strong> — vocês dois podem editar.
          </p>
        ...
        ` : `
          ...
          <div class="field">
            ...
            <input type="text" id="share-cpf" inputmode="numeric" maxlength="14" placeholder="000.000.000-00" autocomplete="off">
          </div>
        `}
      </div>
      <div class="modal-foot">
...
function attachEvents(){
  ...
  const shareCpfEl = document.getElementById('share-cpf');
  if(shareCpfEl){
    shareCpfEl.addEventListener('input', e=>{ e.target.value = formatCPF(e.target.value); });
    shareCpfEl.focus();
  }
  ...
  if(confirmShareBtn) confirmShareBtn.addEventListener('click', async ()=>{
    const cpfDigits = document.getElementById('share-cpf').value.replace(/\D/g,'');
    if(!isValidCPF(cpfDigits)){ shareError = 'CPF inválido.'; render(); return; }
    confirmShareBtn.disabled = true;
    confirmShareBtn.textContent = 'Compartilhando...';
    const result = await shareAnimal(shareState.animalId, cpfDigits);
    if(result.ok){
