...
function renderHistoryTab(a, history){
  ...
  ${filtered.length ? `<div class="timeline">${filtered.map(h=>`
    <div class="tl-item">
      ...
      ${h.price ? `<div class="tl-foot">Valor: <span class="mono">${formatCurrency(h.price)}</span></div>` : ''}
  ...
  `).join('')}</div>` : q ? emptyState('🔎','Nenhum resultado para essa busca.') : emptyState('📋','Ainda não há registros de saúde por aqui.')}`;
}
