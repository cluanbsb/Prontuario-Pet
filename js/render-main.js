...
function emptyState(icon, text){
  return `<div class="empty-tab"><span class="empty-icon" aria-hidden="true">${icon}</span>${text}</div>`;
}
...
function renderSidebar(){
  ...
  const pickerList = animals.length
    ...
    : `<div class="empty-sidebar"><span style="display:block;font-size:32px;margin-bottom:8px;" aria-hidden="true">🐾</span>Você ainda não tem nenhum bichinho por aqui. Clique em "Novo animal" para começar!</div>`;
  ...
  <aside class="sidebar">
    <div class="brand">
      ...
      ${userMenuOpen ? `
        ...
        <div class="user-menu">
          ...
          <div class="menu-group-label">Conta</div>
          <button class="menu-item" data-action="open-edit-account">✎ Editar cadastro e senha</button>
          <div class="menu-group-label">Preferências</div>
          <button class="menu-item" data-action="open-customize">🎨 Personalizar</button>
          <div class="menu-group-label">Dados</div>
    ...
    <div class="alert-banner error" role="alert">
      ⚠️ ${escapeHtml(loadError)}
      <button class="link-btn" data-action="retry-load" style="display:block;margin-top:6px;">Tentar de novo</button>
    </div>` : ''}
    ${(() => {
...
function renderDetail(){
  ...
  <main class="detail">
    ...
    <div class="profile-cover">
      <img src="${a.photo}" alt="">
      <div class="profile-cover-scrim"></div>
      <div class="profile-cover-actions">
        <button class="icon-btn-ghost" data-action="edit-animal" aria-label="Editar animal" title="Editar animal">✎</button>
        ${a.isShared ? `<span class="icon-btn-ghost icon-btn-ghost--active" style="cursor:default;" aria-label="Compartilhado por ${escapeAttr(a.ownerEmail||'outro usuário')}" title="Compartilhado por ${escapeAttr(a.ownerEmail||'outro usuário')} · vocês dois podem editar">🔗</span>` : ''}
        ${!a.isShared ? `<button class="icon-btn-ghost ${a.sharedWithUid ? 'icon-btn-ghost--active' : ''}" data-action="open-share" aria-label="${a.sharedWithUid ? `Compartilhado com ${a.sharedWithEmail||''}` : 'Compartilhar com outro usuário'}" title="${a.sharedWithUid ? `Compartilhado com ${a.sharedWithEmail||''} · toque para gerenciar` : 'Compartilhar com outro usuário'}">🔗</button>` : ''}
        ${!a.isShared ? `<button class="icon-btn-ghost" data-action="delete-animal" aria-label="Excluir animal" title="Excluir animal">🗑</button>` : ''}
      </div>
    ...
    <div class="content-panel">
      ${(!a.photo || a.notes) ? `
      <div class="profile-header ${a.photo ? 'profile-header--no-icon' : ''}">
        ${!a.photo ? `<div class="profile-icon">${SPECIES_ICON[a.species]||'🐾'}</div>` : ''}
        ...
        <div class="profile-actions">
          ...
          ${a.isShared ? `<span class="icon-btn icon-btn--active" style="cursor:default;" aria-label="Compartilhado por ${escapeAttr(a.ownerEmail||'outro usuário')}" title="Compartilhado por ${escapeAttr(a.ownerEmail||'outro usuário')} · vocês dois podem editar">🔗</span>` : ''}
          ${!a.isShared ? `<button class="icon-btn ${a.sharedWithUid ? 'icon-btn--active' : ''}" data-action="open-share" aria-label="${a.sharedWithUid ? `Compartilhado com ${a.sharedWithEmail||''}` : 'Compartilhar com outro usuário'}" title="${a.sharedWithUid ? `Compartilhado com ${a.sharedWithEmail||''} · toque para gerenciar` : 'Compartilhar com outro usuário'}">🔗</button>` : ''}
...
function renderPerfilTab(a){
  const weightLog = (a.weightLog || []).slice().sort((x,y)=> (y.date||'').localeCompare(x.date||''));
  const currentWeight = weightLog.length ? parseFloat(weightLog[0].weight) : (parseFloat(a.weight) || null);
  const rows = [
    ['Espécie', a.species],
    ['Raça', a.breed || '—'],
  ...
  if(currentWeight) highlightCards.push(`<div class="highlight-card highlight-green"><span class="hc-label">Peso${weightLog.length ? ' · '+fmtDate(weightLog[0].date) : ''}</span><span class="hc-value">${currentWeight} kg</span></div>`);
  if(a.birthDate) highlightCards.push(`<div class="highlight-card highlight-amber"><span class="hc-label">Idade</span><span class="hc-value">${ageFromBirth(a.birthDate)}</span></div>`);
  return `
  ${highlightCards.length ? `<div class="highlight-cards" style="grid-template-columns:repeat(${highlightCards.length},1fr);">${highlightCards.join('')}</div>` : ''}
  <div class="section-head"><h3>Dados cadastrais</h3></div>
  <div class="rec-card" style="display:block;">
    ${rows.map(([label,val])=>`
      <div class="profile-data-row">
        <span class="rec-line" style="margin:0;">${label}</span>
        <span class="mono" style="font-size:12.5px;font-weight:500;color:var(--ink);">${escapeHtml(String(val))}</span>
      </div>
    `).join('')}
  </div>`;
}
...
function renderVaccinesTab(a, vaccines){
  ...
  <div class="section-head">
    <div>
      <h3>Vacinas</h3>
      ${total>0 ? `<p class="tab-total-spent">Total gasto: ${formatCurrency(total)}</p>` : ''}
    </div>
    <div style="display:flex;gap:8px;">
      ${vaccines.length > 3 ? `<button class="btn-icon-toggle ${searchOpen?'active':''}" data-action="toggle-search" aria-label="Buscar" title="Buscar">🔎</button>` : ''}
      ...
      ${!readOnly ? `<button class="btn-add" data-action="add-record" data-entity="vaccine">+ Registrar</button>` : ''}
    </div>
  </div>
  ${vaccines.length > 3 && searchOpen ? `<input type="text" id="record-search" class="search-input" placeholder="Buscar por nome, lote, veterinário..." value="${escapeAttr(recordSearch)}">` : ''}
  ${sorted.length ? sorted.map(v=>{
    ...
    <div class="rec-card">
      <div class="rec-main">
        ...
        ${(v.lot || v.price) ? `<div class="rec-line">${v.lot ? `Lote: <span class="mono">${escapeHtml(v.lot)}</span>` : ''}${v.lot && v.price ? ' · ' : ''}${v.price ? `Valor: <span class="mono">${formatCurrency(v.price)}</span>` : ''}</div>` : ''}
        ${v.vet ? `<div class="rec-line">${escapeHtml(v.vet)}</div>` : ''}
        ${v.notes ? `<div class="rec-notes"><span style="color:var(--ink-soft);font-weight:600;">Obs:</span> ${escapeHtml(v.notes)}</div>` : ''}
      </div>
      ${!readOnly ? `
  ...
  }).join('') : q ? emptyState('🔎','Nenhum resultado para essa busca.') : emptyState('💉','Ainda não há vacinas por aqui. Que tal registrar a primeira?')}`;
}
...
function renderMedsTab(a, meds){
  ...
  <div class="section-head">
    ...
    <div style="display:flex;gap:8px;">
      ${meds.length > 3 ? `<button class="btn-icon-toggle ${searchOpen?'active':''}" data-action="toggle-search" aria-label="Buscar" title="Buscar">🔎</button>` : ''}
      ...
      ${!readOnly ? `<button class="btn-add" data-action="add-record" data-entity="medication">+ Registrar</button>` : ''}
    </div>
  </div>
  ${meds.length > 3 && searchOpen ? `<input type="text" id="record-search" class="search-input" placeholder="Buscar por nome, dose, local..." value="${escapeAttr(recordSearch)}">` : ''}
  ${sorted.length ? sorted.map(m=>{
    ...
    <div class="rec-card">
      <div class="rec-main">
        ...
        ${(m.price || m.purchaseLocation) ? `<div class="rec-line">${m.price ? `Valor: <span class="mono">${formatCurrency(m.price)}</span>` : ''}${m.price && m.purchaseLocation ? ' · ' : ''}${m.purchaseLocation ? `Local da compra: ${escapeHtml(m.purchaseLocation)}` : ''}</div>` : ''}
        ${m.notes ? `<div class="rec-notes"><span style="color:var(--ink-soft);font-weight:600;">Obs:</span> ${escapeHtml(m.notes)}</div>` : ''}
      </div>
      ${!readOnly ? `
      <div class="rec-actions">
  ...
  }).join('') : q ? emptyState('🔎','Nenhum resultado para essa busca.') : emptyState('💊','Ainda não há medicações por aqui.')}`;
}
...
function renderHistoryTab(a, history){
  ...
  const sorted = filtered.slice().sort((x,y)=> (y.date||'').localeCompare(x.date||''));
  const total = history.reduce((sum,h)=> sum + (parseFloat(h.price)||0), 0);
  return `
  <div class="section-head">
    <div>
    ...
    ${!readOnly ? `<div style="display:flex;gap:8px;">
      ${history.length > 3 ? `<button class="btn-icon-toggle ${searchOpen?'active':''}" data-action="toggle-search" aria-label="Buscar" title="Buscar">🔎</button>` : ''}
      ${animals.length>1 && history.length ? `<button class="btn-add" data-action="open-copy" data-entity="healthRecord" title="Copiar registros para outro animal">📋 Copiar</button>` : ''}
      <button class="btn-add" data-action="add-record" data-entity="healthRecord">+ Registrar</button>
    </div>` : ''}
  </div>
  ${history.length > 3 && searchOpen ? `<input type="text" id="record-search" class="search-input" placeholder="Buscar por tipo, descrição, veterinário..." value="${escapeAttr(recordSearch)}">` : ''}
  ${sorted.length ? sorted.map(h=>`
    <div class="rec-card">
      <div class="rec-main">
        <div class="rec-title-row"><span class="rec-title">${escapeHtml(h.type)}</span></div>
        <div class="rec-line">${fmtDate(h.date)}</div>
        ${h.description ? `<div class="rec-notes">${escapeHtml(h.description)}</div>` : ''}
        ${(h.price || h.weight) ? `<div class="rec-line">${h.price ? `Valor: <span class="mono">${formatCurrency(h.price)}</span>` : ''}${h.price && h.weight ? ' · ' : ''}${h.weight ? `Peso: <span class="mono">${h.weight} kg</span>` : ''}</div>` : ''}
        ${h.vet ? `<div class="rec-line">${escapeHtml(h.vet)}</div>` : ''}
      </div>
      ${!readOnly ? `
      <div class="rec-actions">
  ...
  `).join('') : q ? emptyState('🔎','Nenhum resultado para essa busca.') : emptyState('📋','Ainda não há registros de saúde por aqui.')}`;
}
...
function renderDietTab(a){
  ...
  <div class="section-head">
    ...
    <div style="display:flex;gap:8px;">
      ${animals.length>1 && log.length ? `<button class="btn-add" data-action="open-copy" data-entity="weightEntry" title="Copiar registros para outro animal">📋 Copiar</button>` : ''}
      <button class="btn-add" data-action="add-record" data-entity="weightEntry">+ Registrar</button>
    </div>
  </div>
  ...
  `).join('') : emptyState('⚖️','Ainda não há pesagens registradas. Registre a primeira para acompanhar a evolução.')}
  ...
  `).join('') : emptyState('🛒','Ainda não há compras de ração registradas.')}
