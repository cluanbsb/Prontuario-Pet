...
function renderCopyModal(){
  const labels = { vaccine:'vacina(s)', medication:'medicação(ões)', healthRecord:'registro(s) de saúde', weightEntry:'registro(s) de peso' };
  const dateFields = { vaccine:'dateApplied', medication:'startDate', healthRecord:'date', weightEntry:'date' };
  const entityLabel = labels[copyState.entity] || 'registro(s)';
  const dateField = dateFields[copyState.entity] || 'date';
  const listKey = getListKey(copyState.entity);
  const titleOf = r => {
    if(copyState.entity === 'healthRecord') return r.type || 'Registro';
    if(copyState.entity === 'weightEntry') return `${r.weight} kg`;
    return r.name || '';
  };
  const source = getAnimal(selectedId);
  const records = (source[listKey] || []).slice().sort((x,y)=> (y[dateField]||'').localeCompare(x[dateField]||''));
            ...
            ${records.map(r => `
              <label style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;border-bottom:1px solid var(--line);cursor:pointer;">
                ...
                <span style="font-size:13.5px;">
                  <strong>${escapeHtml(titleOf(r))}</strong><br>
    ...
    <div class="modal modal-wide" role="dialog" aria-modal="true">
      ...
      <div class="customize-section">
        ...
        <div class="opacity-row">
          <label for="wallpaper-opacity-slider">Intensidade</label>
          <input type="range" id="wallpaper-opacity-slider" min="0.1" max="1" step="0.05" value="${s.wallpaperOpacity != null ? s.wallpaperOpacity : 1}">
          <span class="mono" id="wallpaper-opacity-value">${Math.round((s.wallpaperOpacity != null ? s.wallpaperOpacity : 1) * 100)}%</span>
        </div>` : ''}
      </div>
