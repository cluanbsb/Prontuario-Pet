...
function attachEvents(){
  ...
  if(opacitySlider){
    opacitySlider.addEventListener('input', ()=>{
      ...
      pendingSettings.wallpaperOpacity = val;
      document.documentElement.style.setProperty('--wallpaper-opacity', val);
  ...
  if(resetCustomizeBtn) resetCustomizeBtn.addEventListener('click', ()=>{
    pendingSettings = { colorTheme:'sapo', font:'classico', wallpaper:'none', wallpaperOpacity:1 };
...
function confirmCropAndUpload(){
  ...
  img.onload = ()=>{
    ...
    try{
      ...
      alert('Não foi possível processar essa imagem. Tente novamente.');
      return;
    }
    const overlay = document.getElementById('crop-overlay');
    if(overlay) overlay.remove();
...
auth.onAuthStateChanged(user=>{
  ...
  if(user){
    ...
    animals = [];
    settings = { colorTheme:'sapo', font:'classico', wallpaper:'none', wallpaperOpacity:1 };
    loaded = false;
    render();
  }
});
