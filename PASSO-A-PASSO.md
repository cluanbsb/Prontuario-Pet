...
5. **Configure as regras de segurança do Firestore**
   ...
   dados (`usuarios`), que um prontuário compartilhado (`compartilhados`)
   só pode ser lido e editado por quem o compartilhou ou por quem o
   recebeu — só quem compartilhou pode criar ou apagar o compartilhamento
   em si — e que a busca de CPF (`cpf_lookup`) pode ser lida por qualquer
   pessoa, mesmo sem estar logada (necessário para o login por CPF
   encontrar o e-mail certo e para o "Esqueci a senha" funcionar), mas só
   o próprio dono pode gravar a própria entrada.

   > **Nota de privacidade:** como a busca por CPF precisa funcionar antes
   > do login, o vínculo "CPF → e-mail" fica publicamente legível no banco
   > (não a senha, nunca). Isso é necessário para este modelo de login
   > funcionar sem um servidor próprio.
   Clique em **Publicar**.
...
Ao criar a conta, há um campo opcional de **e-mail de recuperação**.

- **Se cadastrar o e-mail:** na tela de login, "Esqueci a senha" manda um
  link de redefinição de verdade para esse e-mail (usando o próprio
  Firebase).
- **Se não cadastrar:** a única forma de trocar a senha é pela opção
  "Alterar senha" na tela de login, que pede a senha atual — não tem como
  recuperar o acesso caso ela seja esquecida por completo.
